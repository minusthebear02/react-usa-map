...
class USAMap extends React.Component {
  ...
  buildText = () => {
    ...
    let dataStates = data();
    for (let stateKey in dataStates) {
      const name = <foreignObject key={stateKey} width="25" height="30" x={this.props.customize[stateKey].x} y={this.props.customize[stateKey].y }><div className={`state-name state-${stateKey}`}>{stateKey}<br />{this.props.customize[stateKey].numVotes}</div></foreignObject>;
      names.push(name);
    };
    return names;
  };
